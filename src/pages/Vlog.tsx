...
const Vlog = () => {
  ...
  const videos = [{
    ...
    category: 'Liderança',
    description: 'Disciplina: insights práticos sobre consistência e construção de resultados a partir da rotina e do treino disciplinado.',
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex justify-center gap-4">
              <Button variant="comet" size="lg" asChild>
                <a href="https://www.youtube.com/@institutocomet" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
                  <Youtube className="h-5 w-5" />
                  Inscreva-se no YouTube
                </a>
              </Button>
              <Button variant="outline" size="lg" asChild>
                <a href="https://www.instagram.com/tiagorezeckcomet/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map(category => <button key={category} onClick={() => setSelectedCategory(category)} className={`px-6 py-2 rounded-full font-medium transition-all duration-300 ${selectedCategory === category ? 'bg-gradient-comet text-white shadow-comet' : 'bg-card text-foreground hover:bg-muted border border-border'}`}>
                  {category}
                </button>)}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredVideos.map(video => <div key={video.id} className="bg-card rounded-xl border border-border shadow-card hover:shadow-comet transition-all duration-300 hover:-translate-y-2 overflow-hidden group">
                  {/* Thumbnail */}
                  <div className="relative aspect-video overflow-hidden bg-muted cursor-pointer" onClick={() => setOpenVideoId(video.videoId)}>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-col gap-2">
                      <Button variant="comet" size="sm" className="w-full" onClick={() => setOpenVideoId(video.videoId)}>
                        Assistir Vídeo
                      </Button>
                      
                      <Button variant="outline" size="sm" className="w-full" asChild>
                        <a href={video.youtubeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
            ...
            {filteredVideos.length === 0 && <div className="text-center py-20">
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <Button variant="hero" size="xl" asChild>
              <a href="#contact-form">Quero essas estratégias </a>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section id="contact-form" className="py-20">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto">
            ...
            <ContactForm title="Cadastre-se para receber atualizações" buttonText="Quero receber conteúdos" compact={true} />
          </div>
        </div>
      </section>
      ...
      <Dialog open={!!openVideoId} onOpenChange={() => setOpenVideoId(null)}>
        <DialogContent className="max-w-5xl w-[95vw] h-[80vh] p-0">
          <div className="w-full h-full">
            {openVideoId && <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${openVideoId}?autoplay=1`} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full" />}
          </div>
        </DialogContent>
      </Dialog>
    </div>;
